...
function gosmtp_pro_connection_and_template_settings(){
	...
	if($mailer_count > 1){
		?>
		<tr>
			<th scope="row"><?php esc_html_e('Test Connection', 'gosmtp'); ?></th>
			<td>
				<select name="smtp_test_connection" class="regular-text" rows="10" required>
					<option value="0" selected><?php esc_html_e('Default Connection', 'gosmtp') ?></option>
					<?php
				...
				<p class="description" id="tagline-description"><?php esc_html_e('Select the connection for sending your message', 'gosmtp'); ?></p>
				<p class="description" id="tagline-description"><i><?php esc_html_e('Please note that the Default Connection uses your current SMTP Settings while others use Additional Connections.', 'gosmtp'); ?></i></p>
			</td>
		</tr>
		<?php
	}
	...
	<tr>
		...
		<td>
			...
			<p class="description" id="tagline-description"><?php esc_html_e('Send test email using HTML template instead of custom message.', 'gosmtp'); ?></p>
		</td>
	</tr>
<?php
}
