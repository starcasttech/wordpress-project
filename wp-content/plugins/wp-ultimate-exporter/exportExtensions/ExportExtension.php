...
if (class_exists('\Smackcoders\FCSV\MappingExtension')) {
	...
	class ExportExtension extends \Smackcoders\FCSV\MappingExtension
	{
		...
		public $limit;
		protected static $instance = null, $mapping_instance, $metabox_export, $jet_reviews_export, $jet_book_export, $jetengine_export, $export_handler, $post_export, $woocom_export, $review_export, $ecom_export, $learnpress_export, $wpquery_export, $edd_export,$surecart_export;
		...
		public static function getInstance()
		{
			if (null == self::$instance) {
				...
				ExportExtension::$edd_export = EDDExport::getInstance();
				ExportExtension::$surecart_export = SureCartExport::getInstance();
		...
		public function totalRecords()
		{
			...
			if ($module == 'EDD_CUSTOMERS' || $module == 'EDD_DISCOUNTS' || $module == 'EDD_ORDERS' || $module == 'EDD_DOWNLOADS') {
				...
				if ($module == 'EDD_CUSTOMERS') {
					...
					if (class_exists('\\EDD\\Orders\\Order_Query')) {
						...
						$total = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}edd_discounts");
					}
				} elseif ($module == 'EDD_ORDERS') {
					if (class_exists('\\EDD\\Orders\\Order_Query')) {
						$total = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}edd_orders");
					} else {
						$total = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}posts WHERE post_type = 'edd_payment' AND post_status != 'trash'");
					}
				} elseif ($module == 'EDD_DOWNLOADS') {
			...
			if($optionalType == 'SURECART_PRODUCTS' || $optionalType == 'SURECART_CUSTOMERS' || $optionalType == 'SURECART_COUPONS'){
				if ($optionalType == 'SURECART_PRODUCTS') {
					$optionalType = 'sc_product';
				} elseif ($optionalType == 'SURECART_CUSTOMERS') {
					$optionalType = 'sc_customer';
				} elseif ($optionalType == 'SURECART_COUPONS') {
					$optionalType = 'sc_coupon';
				}
				// error_log('this exporter called'.PHP_EOL);
				if (is_plugin_active('surecart/surecart.php')) {
					...
					if (array_key_exists($optionalType, $model_mapping)) {
						$model_class = $model_mapping[$optionalType];
			...
			// Check if CustomPosts is actually an EDD module
		...
		public function parseData() { error_log("DEBUG: parseData called in ExportExtension.");
			...
			if (!empty($_POST)) {
				...
				// Reassign module for EDD types if sent as CustomPosts to ensure correct flow in exportData()
				if ($this->module == 'CustomPosts' && in_array($this->optionalType, ['EDD_CUSTOMERS', 'EDD_DISCOUNTS', 'EDD_ORDERS', 'EDD_DOWNLOADS'])) {
					$this->module = $this->optionalType;
				}
				if ($this->optionalType == 'SURECART_PRODUCTS' || $this->optionalType == 'SURECART_CUSTOMERS' ||	$this->optionalType == 'SURECART_COUPONS') {
						$this->module = $this->optionalType;
				}
		...
		public function getOptionalType($module)
		{
			if ($module == 'Tags') {
				...
				$optionalType = 'download';
			} elseif ($module == 'EDD_ORDERS') {
				if (class_exists('\\EDD\\Orders\\Order_Query')) {
					$optionalType = 'edd_order';
				} else {
					$optionalType = 'edd_payment';
				}
			} elseif ($module == 'EDD_CUSTOMERS') {
				$optionalType = 'edd_customers';
			} elseif ($module == 'EDD_DISCOUNTS') {
				$optionalType = 'edd_discounts';
...
error_log("DEBUG: Module: " . $this->module . ", OptionalType: " . $this->optionalType);
			if (!empty($recordsToBeExport)) {
				foreach ($recordsToBeExport as $postId) { error_log("DEBUG: Processing postId: " . $postId . " for module: " . $this->module);
					...
					if ($this->module == 'EDD_DOWNLOADS')
						ExportExtension::$edd_export->getEDDDownloadDataMaster($postId);
					if ($this->module == 'EDD_ORDERS')
					...
					if ($this->module == 'SURECART_PRODUCTS' || $this->optionalType == 'SURECART_PRODUCTS')
						ExportExtension::$surecart_export->getSureCartProductDataMaster($postId);
					if ($this->module == 'SURECART_CUSTOMERS' || $this->optionalType == 'SURECART_CUSTOMERS')
						ExportExtension::$surecart_export->getSureCartCustomerDataMaster($postId);
					if ($this->module == 'SURECART_COUPONS' || $this->optionalType == 'SURECART_COUPONS')
						ExportExtension::$surecart_export->getSureCartCouponDataMaster($postId);
		...
		public function _enclose_value($value = null)
		{
			if ($value !== null && $value != '') {
				...
				if (
					isset($value) && is_string($value) && preg_match("/" . $delimiter . "|" . $enclosure . "|\n|\r/i", $value) ||
