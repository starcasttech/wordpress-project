...
class PostExport extends ExportExtension
{
	...
	public function getRecordsBasedOnPostTypes($module, $optionalType, $conditions, $offset, $limit, $headers = '')
	{
		...
		if ($module == 'WooCommerceOrders') {
		...
		}elseif ($module == 'SURECART_PRODUCTS') {
				$module = 'sc_product';
		} elseif ($module == 'SURECART_CUSTOMERS') {
			if (class_exists('\\SureCart\\Models\\Customer')) {
				try {
					$collection = \SureCart\Models\Customer::paginate(['per_page' => 1]);
					$total_count = $collection->total();
					if (isset(self::$export_instance)) {
						self::$export_instance->totalRowCount = $total_count;
					}

					$customers = \SureCart\Models\Customer::where(['limit' => $limit, 'offset' => $offset])->get();
					$result = [];
					if (!empty($customers)) {
			...
			if (class_exists('\\SureCart\\Models\\Coupon')) {
				try {
					$collection = \SureCart\Models\Coupon::paginate(['per_page' => 1]);
					$total_count = $collection->total();
					if (isset(self::$export_instance)) {
						self::$export_instance->totalRowCount = $total_count;
					}

					$coupons = \SureCart\Models\Coupon::where(['limit' => $limit, 'offset' => $offset])->get();
					$result = [];
					if (!empty($coupons)) {
