...
class Wt_Import_Export_For_Woo_Product_Export {
	...
	$only_last_modified = !empty($form_data['advanced_form_data']['wt_iew_only_last_modified']) ? $form_data['advanced_form_data']['wt_iew_only_last_modified'] : 0;
        ...
        $this->export_shortcodes = (!empty($form_data['advanced_form_data']['wt_iew_export_shortcode_tohtml']) && ( $form_data['advanced_form_data']['wt_iew_export_shortcode_tohtml'] == 'Yes' || $form_data['advanced_form_data']['wt_iew_export_shortcode_tohtml'] == 1 ) ) ? true : false;

        $this->export_images_zip = (!empty($form_data['advanced_form_data']['wt_iew_image_export']) && ($form_data['advanced_form_data']['wt_iew_image_export'] == 'Yes' || $form_data['advanced_form_data']['wt_iew_image_export'] == '1')) ? true : false;
        ...
        if ($batch_offset < $export_limit)
        {
            ...
            $args = apply_filters("woocommerce_csv_product_export_args", $args, $form_data);

            $products = wc_get_products($args);

            $products = apply_filters('wt_ier_product_ids_befor_export', $products, $form_data );
            $total_products=0;
                    ...
                    if ($product->is_type('variable') || $product->has_child()) {
                        ...
                        if (!empty($children_ids)) {
                            foreach ($children_ids as $id) {                                
                                if(!in_array($id, $products_ids)){  // skipping if already processed in $products_ids                              
                                    if('export_image' == $step){                        
                                        if ($id) {                            
                                        ...
                                        if( isset( $args['stock_status'] ) && ( 'instock' === $args['stock_status'] ) && ( ( !$variation->is_in_stock() ) || ( $variation->is_on_backorder() ) ) ){
                                            continue;
                                        }                                      
        ...
        foreach ($csv_columns as $column => $value) {
            ...
            if (!$export_columns || in_array($value, $export_columns) || in_array($column, $export_columns)) {
                ...
                if ('meta:' == substr($column, 0, 5)) {
                    ...
                    } else if (get_post_meta($product->ID, $product_meta, true)) {
                        $meta_data_raw = get_post_meta($product->ID, $product_meta, true);
                        if (is_array($meta_data_raw)) {
                            $meta_data_raw = serialize($meta_data_raw);
                        }
                        $row[$column] = self::format_data($meta_data_raw);
                    } else {
                        $row[$column] = '';
                    }
                    continue;
