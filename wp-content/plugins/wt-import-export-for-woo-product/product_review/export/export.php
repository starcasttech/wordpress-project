...
//        }
        ...
        foreach ($csv_columns as $column => $value) {
                ...
                if (strpos($column, 'meta:') !== false) {
                    $column_key = trim(str_replace('meta:', '', $column));
                        if (isset($comment->meta->$column_key)) {
                            $cmeta_value = is_array(($comment->meta->$column_key)) ? json_encode(($comment->meta->$column_key)) : ($comment->meta->$column_key);
                            $row[$column] = $cmeta_value;
                        } else {
                            $row[$column] = '';
                        }
                    continue;
