			...
			if($history_item) //history item exists
			{
				...
				if($action_type=='import' && Wt_Import_Export_For_Woo_Admin::module_exists($action_type))
				{
					...
					{						
						...
						$batch_count = apply_filters('wt_ier_load_history_details_batch_count', 50);
						$response=$reader->get_data_as_batch($log_file_path, $offset, $batch_count);
		...
		if($record_count>=1)
	 	{
			...
			$latest_finished_entries = $wpdb->get_results( $wpdb->prepare( "SELECT id FROM {$wpdb->prefix}wt_iew_action_history WHERE status IN(0,1) ORDER BY id DESC LIMIT 0, %d",  $limit_record_count ), ARRAY_A );
	...
	*/
	public static function create_history_entry($file_name, $form_data, $to_process, $action)
	{
		...
		$insert_data=array(
			...
			'created_at'=>current_time( 'timestamp' ), //craeted time
